...
#pragma shader_feature_local _ _ALPHATEST_ON _ALPHABLEND_ON _ADDITIVE_ON
...
#if defined(_ALPHABLEND_ON) || defined(_ADDITIVE_ON)
#define _TRANSPARENT
#undef _ALPHA_CLIP
#undef UNITY_UI_ALPHACLIP
#else
...
Varyings VertexStage(Attributes input)
{
...
#if defined(_VERTEX_EXTRUSION_SMOOTH_NORMALS)
#if defined(_URP)
    worldVertexPosition += TransformObjectToWorldNormal(input.uv2.xyz * float3(output.scale.x, output.scale.y, 1.0)) * _VertexExtrusionValue;
#else
    worldVertexPosition += UnityObjectToWorldNormal(input.uv2.xyz * float3(output.scale.x, output.scale.y, 1.0)) * _VertexExtrusionValue;
#endif
#else
    ...
    for (int hoverLightIndex = 0; hoverLightIndex < HOVER_LIGHT_COUNT; ++hoverLightIndex)
    {
        ...
        fadeDistance = min(fadeDistance, GTNearLightDistance(_HoverLightData[dataIndex], output.worldPosition));
    }
    ...
    for (int proximityLightIndex = 0; proximityLightIndex < PROXIMITY_LIGHT_COUNT; ++proximityLightIndex)
    {
        ...
        fadeDistance = min(fadeDistance, GTNearLightDistance(_ProximityLightData[dataIndex], output.worldPosition));
    }
#else
...
// Texturing.
    ...
    albedo *= GTUnityUIClipRect(input.localPosition.xy, _ClipRect, _ClipRectRadii);
#endif
